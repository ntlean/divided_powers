...
* `LinearMap.onSup_equiv` : the bijection between the set of linear maps `M + N →ₗ[A] Y` and the
  set of pairs of linear maps `(M →ₗ[A] Y) × (N →ₗ[A] Y))` that agree on the intersection `M ⊓ N`.
* `LinearMap.onSup_lequiv` : the `A`-linear equivalence between the module of linear maps
