...
TODO:
If `K` is a `p`-adic local field with ring of integers `R` and uniformizer `π` such that
...
theorem padicValNat_factorial_le (n : ℕ) :
    padicValNat p n.factorial ≤ n := by
  by_cases hn : n = 0
  · simp [hn]
...
variable {A B : Type*} [CommSemiring A] [CommSemiring B]
  (I : Ideal A) [DecidablePred fun x ↦ x ∈ I] (J : Ideal B)
