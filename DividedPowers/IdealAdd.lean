...
theorem dpow_add (hIJ : ∀ {n : ℕ}, ∀ a ∈ I ⊓ J, hI.dpow n a = hJ.dpow n a)
  ...
  simp_rw [← sum_antidiagonalTwoTwo_eq (f := fun (i, l) ↦
    hI.dpow i.1 a * hI.dpow l.1 a' * hJ.dpow i.2 b * hJ.dpow l.2 b'), ← sum_antidiagonalTwoTwo_eq
      (f := fun (i, l) ↦ hI.dpow i.1 a * hI.dpow i.2 a' * hJ.dpow l.1 b * hJ.dpow l.2 b')]
  let i : (ℕ × ℕ) × (ℕ × ℕ) → (ℕ × ℕ) × (ℕ × ℕ) := fun (u, v) ↦ ((u.1, v.1), (u.2, v.2))
  have hi (a) (ha : a ∈ antidiagonalTwoTwo n) : i a ∈ antidiagonalTwoTwo n := by
    simp only [mem_antidiagonalTwoTwo] at ha ⊢
